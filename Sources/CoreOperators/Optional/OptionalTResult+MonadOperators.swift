...
public func >>- <A, B, E: Error>(_ opt: Result<A, E>?, _ fn: @escaping (A) -> Result<B, E>?) -> Result<B, E>? {
    opt.flatMapT(fn)
}

// (-<<) :: (a -> Result<b,e>?) -> Result<a,e>? -> Result<b,e>?
public func -<< <A, B, E: Error>(_ fn: @escaping (A) -> Result<B, E>?, _ opt: Result<A, E>?) -> Result<B, E>? {
    opt.flatMapT(fn)
}
