...
public func >>- <S, A, B>(_ stateful: Stateful<S, [A]>, _ fn: @escaping @Sendable (A) -> Stateful<S, [B]>) -> Stateful<S, [B]> {
    stateful.flatMapT(fn)
}

/// (-<<) :: (a -> Stateful<s, [b]>) -> Stateful<s, [a]> -> Stateful<s, [b]>
public func -<< <S, A, B>(_ fn: @escaping @Sendable (A) -> Stateful<S, [B]>, _ stateful: Stateful<S, [A]>) -> Stateful<S, [B]> {
    stateful.flatMapT(fn)
}
