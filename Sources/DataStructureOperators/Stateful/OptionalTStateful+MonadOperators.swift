...
public func >>- <S, A, B>(_ opt: Stateful<S, A>?, _ fn: @escaping (A) -> Stateful<S, B>) -> Stateful<S, B>? {
    opt.flatMapT(fn)
}

// (-<<) :: (a -> Stateful<s, b>) -> Stateful<s, a>? -> Stateful<s, b>?
public func -<< <S, A, B>(_ fn: @escaping (A) -> Stateful<S, B>, _ opt: Stateful<S, A>?) -> Stateful<S, B>? {
    opt.flatMapT(fn)
}
