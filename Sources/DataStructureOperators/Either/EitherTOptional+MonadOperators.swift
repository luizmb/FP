...
public func >>- <L, A, B>(_ either: Either<L, A?>, _ fn: @escaping @Sendable (A) -> Either<L, B?>) -> Either<L, B?> {
    flatMapTEitherOptional(either, fn)
}

/// (-<<) :: (a -> Either<l,b?>) -> Either<l,a?> -> Either<l,b?>
public func -<< <L, A, B>(_ fn: @escaping @Sendable (A) -> Either<L, B?>, _ either: Either<L, A?>) -> Either<L, B?> {
    flatMapTEitherOptional(either, fn)
}
