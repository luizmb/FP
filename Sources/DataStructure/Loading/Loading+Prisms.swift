...
//
// The macro itself can't be applied because Loading is a generic type and Swift forbids
// `static let` stored properties in generic contexts. We use `static var` (computed) so
// the surface is identical for callers; the only structural difference is the storage
// strategy.
