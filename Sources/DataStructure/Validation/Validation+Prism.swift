...
// prism (`Validation.prism.success.preview(x)`), `\.success`, or the plain `validation.success`
// property.
...
public extension Validation {
    ...
    /// The accumulated error(s), or `nil` if `self` is `.success`. Delegates to `Self.prism.failure`.
    var failure: E? { Self.prism.failure.preview(self) }
    /// The successful value, or `nil` if `self` is `.failure`. Delegates to `Self.prism.success`.
    var success: A? { Self.prism.success.preview(self) }
