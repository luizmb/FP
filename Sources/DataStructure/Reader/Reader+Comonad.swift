...
public extension Reader where Environment: Monoid {
    ...
    func extend<B>(_ f: @escaping (Reader<Environment, Output>) -> B) -> Reader<Environment, B> {
    ...
    func coflatMap<B>(_ f: @escaping (Reader<Environment, Output>) -> B) -> Reader<Environment, B> {
        extend(f)
    }
    ...
    static func extend<B>(
        _ f: @escaping (Reader<Environment, Output>) -> B
...
public func extract<Env: Monoid, A>(_ reader: Reader<Env, A>) -> A {
    reader.extract
}

/// extend :: (Reader Env A -> B) -> Reader Env A -> Reader Env B  (requires Env: Monoid)
public func extend<Env: Monoid, A, B>(
...
public func duplicate<Env: Monoid, A>(_ reader: Reader<Env, A>) -> Reader<Env, Reader<Env, A>> {
    reader.duplicate
}
