...
func monadOptional() {
    ...
    Int?.bind(positive)(x) // Optional(5)
    Int?.bind(positive)(nil) // nil
    ...
    Int?.join(.some(.some(42))) // Optional(42)
    Int?.join(.some(Int?.none)) // nil
...
func monadPublisher() async {
    ...
    for await v in bound.values {
        r2.append(v)
    }
    for await v in piped.values {
        r3.append(v)
    }
    for await v in pipedR.values {
        r4.append(v)
    }
    r1 // ["User #42"]
    r2 // ["User #42"]
    ...
    for await v in chain("hello").eraseToAnyPublisher().values {
        r5.append(v)
    }
    for await v in chainOp("hello").eraseToAnyPublisher().values {
        r6.append(v)
    }
    for await v in chainR("hello").eraseToAnyPublisher().values {
        r7.append(v)
    }
    r5 // ["len:5"]
    r6 // ["len:5"]
