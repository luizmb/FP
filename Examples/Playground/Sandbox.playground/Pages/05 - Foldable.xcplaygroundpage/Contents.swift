...
func learnFoldLeft() {
    ...
    // Sum, product
    Array<Int>.foldLeft(0, +)(nums)                   // 15
    Array<Int>.foldLeft(1, *)(nums)                   // 120

    // Build a string left-to-right
    Array<Int>.foldLeft("") { acc, n in acc + "\(n)" }(nums)  // "12345"

    // Max without using max()
    Array<Int>.foldLeft(Int.min, { Swift.max($0, $1) })(nums)  // 5
...
func learnFoldRight() {
    ...
    Array<Int>.foldRight(+, 0)(nums)                  // 15

    // Build list in original order (prepend = correct order)
    Array<Int>.foldRight({ x, acc in [x] + acc }, [])(nums)   // [1,2,3,4,5]
...
func learnFoldMap() {
    ...
    Array<Int>.foldMap { "\($0)" }(nums)              // "12345"
    Array<String>.foldMap { $0.uppercased() }(words)  // "HELLOWORLD!"

    // Count positives: map to Int.Monoids.Sum (0 or 1), combine via +
    let mixed = [-1, 2, -3, 4, 5]
    Array<Int>.foldMap { n in Int.Monoids.Sum(n > 0 ? 1 : 0) }(mixed)
    // Sum(3)
