...
@Suite struct DeferredTaskTransformerFunctorTests {
    ...
    @Test func deferredTaskTArrayForwardFmap() async {
        ...
        let result = await ({ $0 * 2 } <£^> task).run()
        #expect(result == [2, 4, 6])
    }

    @Test func deferredTaskTArrayFlippedFmap() async {
        let task = DeferredTask<[Int]> { [1, 2, 3] }
        let result = await (task <&^> { $0 * 2 }).run()
        #expect(result == [2, 4, 6])
    }

    // MARK: - DeferredTask<A?>

    @Test func deferredTaskTOptionalForwardFmap() async {
        let task = DeferredTask<Int?> { 5 }
        let result = await ({ $0 * 2 } <£^> task).run()
        #expect(result == 10)
    }

    @Test func deferredTaskTOptionalFlippedFmap() async {
        let task = DeferredTask<Int?> { 5 }
        let result = await (task <&^> { $0 * 2 }).run()
        #expect(result == 10)
    }

    @Test func deferredTaskTOptionalNilPassthrough() async {
        let task = DeferredTask<Int?> { nil }
        let result = await ({ $0 * 2 } <£^> task).run()
        #expect(result == nil)
    }

    // MARK: - DeferredTask<Result<A,E>>

    @Test func deferredTaskTResultForwardFmap() async throws {
        enum Err: Error { case fail }
        let task = DeferredTask<Result<Int, Err>> { .success(4) }
        let result = await ({ $0 * 2 } <£^> task).run()
        #expect(try result.get() == 8)
    }

    @Test func deferredTaskTResultFlippedFmap() async throws {
        enum Err: Error { case fail }
        let task = DeferredTask<Result<Int, Err>> { .success(4) }
        let result = await (task <&^> { $0 * 2 }).run()
        #expect(try result.get() == 8)
...
@Suite struct DeferredStreamTransformerFunctorTests {
    ...
    @Test func deferredStreamTArrayForwardFmap() async {
        let stream = DeferredStream<[Int]> {
            AsyncStream { c in c.yield([1, 2, 3]); c.finish() }
        }
        var result: [Int] = []
        for await v in ({ $0 * 2 } <£^> stream) { result = v }
        #expect(result == [2, 4, 6])
    }

    @Test func deferredStreamTArrayFlippedFmap() async {
        let stream = DeferredStream<[Int]> {
            AsyncStream { c in c.yield([1, 2, 3]); c.finish() }
        }
        var result: [Int] = []
        for await v in (stream <&^> { $0 * 2 }) { result = v }
        #expect(result == [2, 4, 6])
    }

    // MARK: - DeferredStream<A?>

    @Test func deferredStreamTOptionalForwardFmap() async {
        let stream = DeferredStream<Int?> {
            AsyncStream { c in c.yield(5); c.yield(nil); c.finish() }
        }
        var results: [Int?] = []
        for await v in ({ $0 * 2 } <£^> stream) { results.append(v) }
        #expect(results == [10, nil])
    }

    @Test func deferredStreamTOptionalFlippedFmap() async {
        let stream = DeferredStream<Int?> {
            AsyncStream { c in c.yield(5); c.yield(nil); c.finish() }
        }
        var results: [Int?] = []
        for await v in (stream <&^> { $0 * 2 }) { results.append(v) }
        #expect(results == [10, nil])
    }

    // MARK: - DeferredStream<Result<A,E>>

    @Test func deferredStreamTResultForwardFmap() async throws {
        enum Err: Error, Equatable { case fail }
        let stream = DeferredStream<Result<Int, Err>> {
            AsyncStream { c in c.yield(.success(3)); c.yield(.failure(.fail)); c.finish() }
        }
        var results: [Result<Int, Err>] = []
        for await v in ({ $0 * 2 } <£^> stream) { results.append(v) }
        #expect(results == [.success(6), .failure(.fail)])
    ...
    @Test func deferredStreamTResultFlippedFmap() async throws {
        enum Err: Error, Equatable { case fail }
        let stream = DeferredStream<Result<Int, Err>> {
            AsyncStream { c in c.yield(.success(3)); c.yield(.failure(.fail)); c.finish() }
        }
